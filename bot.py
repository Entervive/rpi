...
token = ("NzkzNjI1MzAxNTMxMzYxMzQw.X-u_KA.ITUH0KNHn9gPTCTg_bM18bgWCeA")
...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        if data is None:
            raise YTDLError('Nic nie znaleziono `{}`'.format(search))
...
class Song:
    ...
    def create_embed(self):
        embed = (discord.Embed(title='Teraz leci', description='```css\n{0.source.title}\n```'.format(self), color=discord.Color.blurple())
                .add_field(name='Czas', value=self.source.duration)
                .add_field(name='Wykonawca komendy', value=self.requester.mention)
                .add_field(name='Wykonawca', value='[{0.source.uploader}]({0.source.uploader_url})'.format(self))
...
class Music(commands.Cog):
    ...
    def cog_check(self, ctx: commands.Context):
        if not ctx.guild:
            raise commands.NoPrivateMessage('Nie tutaj.')
    ...
    async def cog_command_error(self, ctx: commands.Context, error: commands.CommandError):
        await ctx.send('Error napisz do Entervive#1399: {}'.format(str(error)))
    ...
    async def _summon(self, ctx: commands.Context, *, channel: discord.VoiceChannel = None):
        ...
        if not channel and not ctx.author.voice:
            raise VoiceError('Nie jesteś na kanale albo nie mogę dołaczyć')
    ...
    async def _leave(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.voice:
            return await ctx.send('Nie jesteś na żadnym kanale')
    ...
    async def _volume(self, ctx: commands.Context, *, volume: int):
        ...
        if 0 > volume > 100:
            return await ctx.send('ustaw pomiędzy 0 a 100')

        ctx.voice_state.volume = volume / 100
        await ctx.send('Głośność ustawiona na {}%'.format(volume))
    ...
    async def _skip(self, ctx: commands.Context):
        ...
        if voter == ctx.voice_state.current.requester:
            ...
            if total_votes >= 3:
                ...
                await ctx.send('Skip **{}/3**'.format(total_votes))

        else:
            await ctx.send('Już wpisałeś skip.')
    ...
    async def _loop(self, ctx: commands.Context):
        ...
        if not ctx.voice_state.is_playing:
            return await ctx.send('Nic teraz nie leci -_-')
    ...
    async def _play(self, ctx: commands.Context, *, search: str):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.send('Error napisz do Entervive#1399: {}'.format(str(e)))
            else:
                if not ctx.voice_state.voice:
    ...
    async def ensure_voice_state(self, ctx: commands.Context):
        if not ctx.author.voice or not ctx.author.voice.channel:
            raise commands.CommandError('Nie jesteś na żadnym kanale.')

        if ctx.voice_client:
            if ctx.voice_client.channel != ctx.author.voice.channel:
                raise commands.CommandError('Bot jest na kanale.')


bot = commands.Bot(command_prefix='?', case_insensitive=True, description="Enter music")
...
bot.run("NzkzNjI1MzAxNTMxMzYxMzQw.X-u_KA.ITUH0KNHn9gPTCTg_bM18bgWCeA")